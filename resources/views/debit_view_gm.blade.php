...
@foreach($debit as $debits)
 ...
 <td>{{$debits->account_name}}</td>
  ...
  <td >{{$debits->taka_in_word}}</td>
   ...
   @if($debits->status == 2)
    <td class="action_td{{$debits->debit_id}}"><a onclick="debit_function_check({{$debits->debit_id}},1)"><i  class="fa fa-check"></i></a><a onclick="debit_function_check({{$debits->debit_id}},0)"> <i class="fa fa-times"></i></a>
    </td>
    @elseif($debits->status == 3 || $debits->status == 4)
...
<script type="text/javascript">
  function debit_function_check(debit_id,status){    
  
    $.ajax({
      url: "{{url('/debit_function_check')}}",
      type: 'GET',
