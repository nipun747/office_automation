...
<table class="table table-striped">
  ...
  @foreach($hrlineDuty as $catagory)
  <tr>
    ...
    @if($catagory->status == 3)
    <td class="action_td{{$catagory->leave_id}}"><a onclick="hrlineacceptReject_duty({{$catagory->leave_id}},1)"><i  class="fa fa-check"></i></a><a onclick="hrlineacceptReject_duty({{$catagory->leave_id}},0)"> <i class="fa fa-times"></i></a>
    </td>
    @else
...
<script type="text/javascript">
  function hrlineacceptReject_duty(leave_id,status){    
  
    $.ajax({
      url: "{{url('/hrlineacceptReject_Duty')}}",
      type: 'GET',
